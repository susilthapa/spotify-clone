...
const refreshAccessToken = async (token) => {
  try {
    spotifyApi.setAccessToken(token.accessToekn)
    spotifyApi.setRefreshToken(token.refreshToken)

    const { body: refreshedToken } = await spotifyApi.refreshAccessToken()
    console.log('REFRESHED TOEKN IS = ', refreshedToken)

    return {
      ...token,
      accessToekn: refreshedToken.access_token,
  ...
  callbacks: {
    async jwt({ token, account, user }) {
      ...
      if (account && user) {
        return {
          ...token,
          accessToekn: account.access_token,
    ...
    async session({ session, token }) {
      session.user.accessToekn = token.accessToekn
